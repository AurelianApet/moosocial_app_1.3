			...
			<div class="comment hasDelLink">
				...
				<div class="comment_like_dislike date">
					...
					<?php if (empty($comment_type)): ?>
						<a href="javascript:void(0)" data-id="<?php echo $comment['Comment']['id']?>" data-type="comment" data-status="1" id="comment_l_<?php echo $comment['Comment']['id']?>" class="comment-thumb likeActivity <?php if ( !empty( $uid ) && !empty( $data['comment_likes'][$comment['Comment']['id']] ) ): ?>active<?php endif; ?>"><i class="material-icons md-18">thumb_up</i></a>
						<?php
						$this->MooPopup->tag(array(
