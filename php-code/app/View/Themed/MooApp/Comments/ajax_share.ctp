	...
	<li class="slide" id="itemcomment_<?php echo $comment['Comment']['id']?>" style="position: relative">
		...
		<?php if ( $comment['Comment']['type'] != APP_CONVERSATION ): ?>
			<div class="dropdown edit-post-icon comment-option">
				<a data-toggle="dropdown" id="itemcomment_edit_<?php echo $comment['Comment']['id']?>" class="cross-icon">
					<i class="material-icons">more_vert</i>
				</a>
				<ul class="dropdown-menu" for="itemcomment_edit_<?php echo $comment['Comment']['id']?>">
					...
					<li>
						<a href="javascript:void(0)" data-id="<?php echo $comment['Comment']['id']?>" data-photo-comment="0" class="removeItemComment">
		...
		<span class="feed-time date">
			...
			<?php if ( $comment['Comment']['type'] != APP_CONVERSATION ): ?>
				...
				<a href="javascript:void(0)" data-id="<?php echo $comment['Comment']['id']?>" data-type="comment" data-status="1" id="comment_l_<?php echo $comment['Comment']['id']?>" class="comment-thumb likeActivity"><i class="material-icons md-18">thumb_up</i></a><span id="comment_like_<?php echo $comment['Comment']['id']?>">0</span>
                    <?php if(empty($hide_dislike)): ?>
					<a href="javascript:void(0)" data-id="<?php echo $comment['Comment']['id']?>" data-type="comment" data-status="0" id="comment_d_<?php echo $comment['Comment']['id']?>" class="comment-thumb likeActivity"><i class="material-icons md-18">thumb_down</i></a><span id="comment_dislike_<?php echo $comment['Comment']['id']?>">0</span>
				<?php endif; ?>
			<?php endif; ?>
		</span>
