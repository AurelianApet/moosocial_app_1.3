...
(function (root, factory) {
    ...
    var init = function(){
        ...
        $('#global-search-filters li').click(function(){
            ...
            $('#center_app').load( encodeURI( data.url ), {noCache: 1}, function(response){
                obj.spin(false);
                mooResponsive.init();
    ...
    var hashInit = function(params){
        ...
        if(tabs != '')
        {
            if ($("#filter-"+tabs).length > 0)
            {
                ...
                $('#center_app').html(mooPhrase.__('loading'));
                $('#center_app').load( $('#filter-'+tabs).attr('data-url'), function(response){
