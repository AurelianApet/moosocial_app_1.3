        ...
        if (uid != '' || $("#tag-name").val() != '')
        {
            ...
            $.post(mooConfig.url.base + '/photos/ajax_tag',
                ...
                if (json.result == 1)
                {
                    ...
                    $("#tags").append('<span class="photoDetailTags" id="hotspot-item-'+tagCounter+'-'+json.id+'" data-tag-id="'+json.id+'" id="hotspot-item-' + tagCounter + '">' + tagValue + '<a class="photoDetailRemoveTags" data-id="'+json.id+'" href="javascript:void(0)"><i class="icon-delete cross-icon-sm"></i></a></span>');
