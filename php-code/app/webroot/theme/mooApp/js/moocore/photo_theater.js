...
(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        // AMD
        define(['jquery', 'mooPhrase', 'mooMention', 'mooEmoji', 'mooBehavior', 'mooAlert', 'elastislide'], factory);
...
}(this, function ($, mooPhrase, mooMention, mooEmoji, mooBehavior, mooAlert) {
    ...
    {
        $.post(mooConfig.url.base + "/photos/ajax_thumb_theater/" + photo_id + "/" + page, function (data) {
        ...
        if (uid != '' || $("#tag-name").val() != '')
        {
            ...
            $.post(mooConfig.url.base + '/photos/ajax_tag', {photo_id: photo_id, uid: uid, value: $("#tag-name").val(), style: style}, function (data) {
                ...
                if (json.result == 1)
                {
                    ...
                    $("#tags").append('<span id="hotspot-item-0-' + json.id + '" data-tag-id="' + json.id + '" class="photoDetailTags">' + tagValue + '<a href="javascript:void(0)" class="photoDetailRemoveTags" data-id="' + json.id + '"><i class="icon-delete cross-icon-sm"></i></a></span>');
